...
har_regenerate_agent_env_file() {
  ...
  # Shared local Supabase stack — resolved once by setup-infra.sh, reused by
  # every agent slot (same URL/keys for all of them).
  local supabase_state="${SCRIPT_DIR}/state/supabase.env"
  if [ -f "$supabase_state" ]; then
    # shellcheck source=/dev/null
    source "$supabase_state"
  fi
  ...
  SUPABASE_DB_URL="${SUPABASE_DB_URL:-}" \
    envsubst '${AGENT_ID} ${API_PORT} ${FE_PORT} ${DEBUG_PORT} ${REPO_ROOT} ${SUPABASE_URL} ${SUPABASE_PUBLISHABLE_KEY} ${SUPABASE_SECRET_KEY} ${SUPABASE_DB_URL}' \
    < "$template" > "$env_file"
...
' "$reg" 2>/dev/null || true)"
  ...
  local supabase_state="${SCRIPT_DIR}/state/supabase.env"
  [ -f "$supabase_state" ] && source "$supabase_state"
  DB_PORT="$(node -e 'try{const u=new URL(process.env.SUPABASE_DB_URL||"");process.stdout.write(u.port||"54322")}catch{process.stdout.write("54322")}' 2>/dev/null)"
  export FE_PORT API_PORT DEBUG_PORT DB_PORT
}
