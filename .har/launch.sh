...
# Supabase's DB port is fixed by supabase/config.toml (shared, not per-slot).
# Read the last resolved value if present so SLOT_PORTS_JSON always has a
# value, even before setup-infra.sh (re-)runs below.
[ -f "$SCRIPT_DIR/state/supabase.env" ] && source "$SCRIPT_DIR/state/supabase.env"
DB_PORT="$(node -e 'try{const u=new URL(process.env.SUPABASE_DB_URL||"");process.stdout.write(u.port||"54322")}catch{process.stdout.write("54322")}' 2>/dev/null || echo 54322)"
...
# Ensure the shared local Supabase stack is running (resolves URLs/keys into
# .har/state/supabase.env). ONE instance serves every agent slot — there is no
# per-slot database or project to create here.
"$SCRIPT_DIR/setup-infra.sh"
# shellcheck source=/dev/null
source "$SCRIPT_DIR/state/supabase.env"
DB_PORT="$(node -e 'try{const u=new URL(process.env.SUPABASE_DB_URL||"");process.stdout.write(u.port||"54322")}catch{process.stdout.write("54322")}' 2>/dev/null || echo 54322)"
...
if [ -n "${HARNESS_HEALTH_CHECK_PATH:-}" ]; then
  HEALTH_URL="http://localhost:${FE_PORT}${HARNESS_HEALTH_CHECK_PATH}"
...
log "  Frontend:  http://localhost:${FE_PORT}  (single Next.js server — API_PORT is the same port)"
log "  Supabase:  ${SUPABASE_URL:-not resolved} (shared by every slot — Studio: ${SUPABASE_STUDIO_URL:-n/a}, Mailpit: ${SUPABASE_MAILPIT_URL:-n/a})"
log ""
